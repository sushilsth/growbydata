...
add_action('wp_ajax_nopriv_ad_intelligence_insights_form', 'ad_intelligence_insights_form_func');
function ad_intelligence_insights_form_func() {
   
    $output = '<script>
    hbspt.forms.create('.includeWithVariables("hubspotForms.php", array("formName"=>"Ad Intelligence Sign up Form"), false) .')
        </script>';
    echo $output;
    die;
} 
...
function custom_store_search_query( $query ) {
    if ( $query->is_search() && $query->is_main_query() && !empty($query->query['s'])) {
        ...
        $result = $wpdb->get_results($wpdb->prepare($parsed_search_query));
        if($result[0]->count >=1 ){
            //Save Keyword
            $ip = get_the_user_ip();  
...
// blog footer slider
add_action ('edited_category', 'update_category_function', 10,1);
add_action('create_category', 'update_category_function');
add_action( 'delete_category', 'update_category_function');
function update_category_function(){
    // Get All category
    ...
    $content='<div class="gbd-blog-bot-slider"><div class="container"><div class="slider-blog-post">';
    ...
    foreach ($category_detail as $cat) {
     ...
     ?>
                ...
                <?php $content .= "<div>".get_field('blog_category_slider_content', 'category_' . $cat->term_id) . "</div>"; ?>
            <?php
        }    
    }     
    $content.='</div></div></div>';
    //Save file    
    if(!file_exists(get_template_directory().'/components/post/') ){
        mkdir(get_template_directory().'/components/post/', 0700);
    }
    $sliderFile = fopen(get_template_directory().'/components/post/blog-footer-static-slider.php', "w") or die("Unable to open file!");
    fwrite($sliderFile,  $content);
    fclose($sliderFile);
}
